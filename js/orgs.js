...
const GPS_SOURCES = [
  ...
  { value: 'fecha_utc_off',  label: 'Fecha/Hora UTC+offset (YYYY-MM-DD HH:MM:SS +00:00)', group: 'GPS' },
  { value: 'fecha_chile',    label: 'Fecha/Hora Chile local (YYYY-MM-DD HH:MM:SS)',         group: 'GPS' },
  { value: 'fecha_chile',    label: 'Fecha/Hora Chile local (DD-MM-YYYY HH:MM:SS)',       group: 'GPS' },
  { value: 'fecha_chile_iso',label: 'Fecha/Hora Chile local ISO (YYYY-MM-DD HH:MM:SS)',   group: 'GPS' },
  ...
  { value: 'Direccion',           label: 'string Dirección de la unidad',              group: 'GPS' },
...
const GPS_TEMPLATES = {
  owl_codelco: {
    label:       'OWL / Codelco QA',
    description: 'Formato Webservice OWL Chile para datos GPS Codelco',
    fields: [
      { apiKey: 'imei',             label: 'IMEI',                  source: 'unit_imei',        required: true  },
      { apiKey: 'patente_vehiculo', label: 'Patente',               source: 'unit_plate',       required: true  },
      { apiKey: 'fecha_gps',        label: 'Fecha GPS',             source: 'fecha_gmt',        required: true  },
      { apiKey: 'fecha_recepcion',  label: 'Fecha recepción',       source: 'fecha_gmt',        required: true  },
      { apiKey: 'latitud',          label: 'Latitud',               source: 'lat',              required: true  },
      { apiKey: 'longitud',         label: 'Longitud',              source: 'lon',              required: true  },
      { apiKey: 'direccion',        label: 'Dirección (°)',         source: 'heading',          required: true  },
      { apiKey: 'velocidad',        label: 'Velocidad',             source: 'speed',            required: true  },
      { apiKey: 'motor',            label: 'Motor (0/1)',           source: 'ignition01',       required: true  },
      { apiKey: 'altura',           label: 'Altitud',               source: 'alt',              required: true  },
      { apiKey: 'satelites',        label: 'Satélites',             source: 'sats',             required: true  },
      { apiKey: 'hdop',             label: 'HDOP',                  source: 'hdop',             required: true  },
  ...
  movup: {
    label:       'MovUP / ALTO',
    description: 'Sistema de Integración de Señales ALTO S.A.',
    fields: [
      { apiKey: 'cod_plate',         label: 'Código vehículo (IMEI)',  source: 'unit_imei',        required: true  },
      { apiKey: 'num_plate',         label: 'Patente',                 source: 'unit_plate',       required: true  },
      { apiKey: 'vehicle_desc',      label: 'Descripción vehículo',    source: 'unit_name',        required: true  },
      { apiKey: 'date_time',         label: 'Fecha/Hora',              source: 'wialon_ts_offset', required: true  },
      { apiKey: 'lat',               label: 'Latitud',                 source: 'lat',              required: true  },
      { apiKey: 'lng',               label: 'Longitud',                source: 'lon',              required: true  },
      { apiKey: 'altitude',          label: 'Altitud',                 source: 'alt',              required: true  },
      { apiKey: 'speed',             label: 'Velocidad',               source: 'speed',            required: true  },
      { apiKey: 'ignition',          label: 'Ignición (0/1)',          source: 'ignition01',       required: true  },
      { apiKey: 'nsat',              label: 'Satélites',               source: 'sats',             required: true  },
      { apiKey: 'hdop',              label: 'HDOP',                    source: 'hdop',             required: true  },
      { apiKey: 'direction',         label: 'Dirección (°)',           source: 'heading',          required: true  },
      { apiKey: 'hdg',               label: 'Rumbo triangulado',       source: 'heading',          required: true  },
      { apiKey: 'provider',          label: 'Proveedor GPS',           source: 'fixed', fixedValue: 'SIGUELOGPS',   required: true  },
      { apiKey: 'vehicle_type',      label: 'Tipo vehículo',           source: 'fixed', fixedValue: '5',            required: true  },
      { apiKey: 'client',            label: 'Cliente',                 source: 'cliente_nombre',   required: true  },
      { apiKey: 'carrier',           label: 'Transportista',           source: 'fixed', fixedValue: '',             required: true  },
      { apiKey: 'provider_register', label: 'Fecha registro',          source: 'wialon_ts_offset', required: true  },
      { apiKey: 'speed_unit',        label: 'Unidad velocidad',        source: 'fixed', fixedValue: '0',            required: true  },
      { apiKey: 'gps_online',        label: 'GPS en línea',            source: 'fixed', fixedValue: '1',            required: true  },
      { apiKey: 'log_gps',           label: 'Es log',                  source: 'fixed', fixedValue: '0',            required: true  },
      { apiKey: 'power',             label: 'Voltaje dispositivo',     source: 'fixed', fixedValue: '0',            required: false },
      { apiKey: 'horometer',         label: 'Horómetro (seg)',         source: 'fixed', fixedValue: '0',            required: false },
      { apiKey: 'odometer',          label: 'Odómetro (metros)',       source: 'odometro',         required: false },
      { apiKey: 'panic',             label: 'Pánico',                  source: 'fixed', fixedValue: '0',            required: false },
      { apiKey: 'batery',            label: 'Voltaje batería',         source: 'fixed', fixedValue: '0',            required: false },
      { apiKey: 'batery_power',      label: 'Batería restante',        source: 'fixed', fixedValue: '0',            required: false },
      { apiKey: 'dinc',              label: 'Distancia incremental',   source: 'fixed', fixedValue: '0',            required: false },
      { apiKey: 'cog',               label: 'Índice precisión GPS',    source: 'hdop',             required: false },
    ],
  },
  gotruck: {
    label:       'GoTruck',
  ...
  sitrack_blueexpress: {
    label:       'Sitrack / Blue Express',
    description: 'Formato WS Receptor Sitrack para Blue Express',
    fields: [
      { apiKey: 'message',        label: 'Message (ambiente)',   source: 'fixed',          fixedValue: 'events.bx.gps.report.test', required: true  },
      { apiKey: 'time',           label: 'Fecha/Hora UTC',       source: 'fecha_hora',                                              required: true  },
      { apiKey: 'eventType',      label: 'Tipo de evento',       source: 'sitrack_evento',                                          required: true  },
      { apiKey: 'gpsDop',         label: 'GPS DOP',              source: 'hdop',                                                    required: true  },
      { apiKey: 'gpsSatellites',  label: 'Satélites',            source: 'sats',                                                    required: true  },
      { apiKey: 'gpsSpeed',       label: 'Velocidad (km/h)',     source: 'speed',                                                   required: true  },
      { apiKey: 'heading',        label: 'Heading (°)',          source: 'heading',                                                 required: true  },
      { apiKey: 'hourmeter',      label: 'Horómetro (h)',        source: 'fixed',          fixedValue: '0',                         required: true  },
      { apiKey: 'ignition',       label: 'Ignición',             source: 'ignition',                                                required: true  },
      { apiKey: 'latitude',       label: 'Latitud',              source: 'lat',                                                     required: true  },
      { apiKey: 'longitude',      label: 'Longitud',             source: 'lon',                                                     required: true  },
      { apiKey: 'odometer',       label: 'Odómetro (km)',        source: 'odometro',                                                required: true  },
      { apiKey: 'rutTransporte',  label: 'RUT transportista',    source: 'cliente_rut',                                             required: true  },
      { apiKey: 'assetId',        label: 'Patente',              source: 'unit_plate',                                              required: true  },
    ],
  },
  pelambres: {
...
function renderPayloadPreview(orgId) {
  ...
  const EXAMPLE = {
    ...
    fecha_chile: (() => { try { const loc=new Date(new Date().toLocaleString('en-US',{timeZone:'America/Santiago'})),p=n=>String(n).padStart(2,'0'); return `${loc.getFullYear()}-${p(loc.getMonth()+1)}-${p(loc.getDate())} ${p(loc.getHours())}:${p(loc.getMinutes())}:${p(loc.getSeconds())}`; } catch(_){return '';} })(),
